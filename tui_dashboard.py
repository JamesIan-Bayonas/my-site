...
if __name__ == "__main__":
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "process-btn":
            ...
            log_window.write(f"[bold green][AI Core][/bold green] Extracted {len(extracted_nodes)} nodes successfully.")
            
            log_window.write("[bold yellow][System][/bold yellow] Feeding entities into Graph Heuristics Engine...")
            math_engine = MathEngine()
            optimized_manifest, total_cost = math_engine.optimize_route(extracted_nodes)
            
            table = Table(title=f"Optimized Path Manifest (Total Weight: {total_cost})", expand=True)
            table.add_column("Stop Sequence", justify="center", style="cyan")
            table.add_column("Target Location Name", style="white")
            table.add_column("Coordinates (Lat, Lng)", style="magenta")
            table.add_column("Operational Priority", justify="center")
            ...
            for step, node in enumerate(optimized_manifest):
                p_val = node.get('priority', 1)
                p_str = "[bold red]🚨 CRITICAL[/bold red]" if p_val == 3 else "[white]📦 Standard[/white]"
                table.add_row(str(step), node['name'], f"{node['lat']}, {node['lng']}", p_str)
            
            log_window.write("\n")
            log_window.write(table)
            
            # --- NEW ARCHIVAL STREAM TRIGGER ---
            log_window.write("\n[bold yellow][System][/bold yellow] Compiling logs to background persistent storage...")
            archiver = DataArchiver()
            _, map_path = archiver.archive_session(optimized_manifest, total_cost)
            
            log_window.write(f"[bold green]✔ Analysis Exported Successfully.[/bold green]")
            log_window.write(f"[bold cyan]📁 Interactive Blueprint Generated:[/bold cyan] {map_path}")